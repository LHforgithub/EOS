...
>用作注明类型可为事件的接收者。可以用于类、接口和方法。类或接口应当是公开的。

EventListenerAttribute添加至类或接口后，即为声明该类、继承该类的类型或继承该接口的类可以作为事件的具体接收者，调用对应方法。（该特性不会被继承，但是不影响继承了已使用该特性的基类或接口的类型也可以作为接收者。）
...
一个类型中，每个事件码只能添加一个用于接收的方法。已在基类中声明为事件方法的那些方法不会继承为事件方法，您只能重新声明这些需要的方法。
